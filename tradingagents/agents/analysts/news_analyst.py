...
def create_news_analyst(llm):
    def news_analyst_node(state):
        ...
        system_message = (
            ...
            "\n  PAYROLL/HCM COMPANIES (ADP, PAYX): Rising interest rates are a TAILWIND, not a"
            "  headwind. These companies hold $25-40B+ in client payroll funds in transit and earn"
            "  interest income on the float. Higher rates = higher float income. Do NOT frame rate"
            "  increases as a risk without acknowledging this structural benefit."
            ...
            "If articles ARE present in the data (even one), IGNORE this section entirely\n"
            "and analyze the articles normally.\n"
            ...
            "\n\n=== PRODUCT & DRUG NAME VERIFICATION (MANDATORY) ===\n"
            "You MUST NOT cite a product name, drug name, brand name, or FDA approval unless"
            " that EXACT name appears in the news articles provided to you. If you cannot find"
            " a specific product name in the source data, write 'product name not confirmed in"
            " available sources' — do NOT guess or generate a plausible-sounding name."
            " Fabricating product names is the single most damaging error a news analyst can make."
            "\n\n=== COMPETITIVE ATTRIBUTION VERIFICATION (MANDATORY) ===\n"
            "Before attributing a competitive threat to the company under review, VERIFY that"
            " the company ACTUALLY sells the product being threatened. Specifically:\n"
            "  - Generic competition for Drug X is relevant ONLY if THIS company sells Drug X.\n"
            "  - If news discusses generics of a competitor's drug (e.g., semaglutide is Novo"
            "    Nordisk's, not Eli Lilly's — LLY's GLP-1 is tirzepatide/Mounjaro/Zepbound),"
            "    do NOT frame it as a direct risk to THIS company.\n"
            "  - State which company ACTUALLY manufactures the drug or product under threat.\n"
            "  - If you are unsure which company owns a specific product, state 'ownership not"
            "    verified' — do NOT assume.\n"
            "  Misattributing a competitor's product risk to the target company is an ANALYTICAL"
            "  ERROR that inverts the competitive framework."
            "\n\nOUTPUT STRUCTURE:"
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    ...
                    ' {{"news_impact": "POSITIVE or NEGATIVE or NEUTRAL or NO_DATA", "confidence": "HIGH or MEDIUM or LOW", "data_gaps": []}}'
                    "\n{system_message}"
