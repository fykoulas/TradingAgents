...
def create_quant_analyst(llm):
    def quant_analyst_node(state):
        ...
        system_message = (
            ...
            "  (i.e., NOT 'N/A'), that value is the AUTHORITATIVE ground truth."
            "\n• You MUST use the verified value in your Implied Expectations table."
            "\n• If you also compute your own reverse DCF (g = WACC − FCF/MarketCap), your"
            "  result MUST match the verified value within 1 percentage point."
            "\n• If your computation differs by >1pp from the verified value, you have an"
            "  arithmetic error or used different inputs. Show BOTH values, state which"
            "  inputs differ, and defer to the verified value for your verdict."
            "\n• NEVER write 'N/A' for Implied FCF Growth when verified data shows a number."
            "  NEVER write a different number without explicit reconciliation."
            "\n"
            ...
            "\n3. **Reverse DCF implied growth** (MANDATORY arithmetic — follow this exact formula):"
            "   Given: Market Cap (or Price × Shares Outstanding), TTM FCF, WACC (use 10%)."
            "   Standard Gordon Growth: Price = FCF / (WACC − g)"
            "   Solve for g: g = WACC − (FCF / Market Cap)"
            "   Example: Market Cap $76B, FCF $3.46B, WACC 10%:"
            "     g = 10% − ($3.46B / $76B) = 10% − 4.55% = 5.45%"
            "   SHOW YOUR ARITHMETIC. Do not skip the calculation."
            "   COMMON ERROR: Forward EPS growth is NOT the implied FCF growth. Forward EPS"
            "   growth is analyst consensus for next year. Implied FCF growth is the perpetual"
            "   rate the current market cap assumes. If your answer equals Forward EPS growth,"
            "   you have made this error — recompute."
            ...
            "\n| Implied FCF Growth (reverse DCF) | X% — GOLDEN VARIABLE: if verified data shows a numeric Implied FCF Growth, copy that value here (it is authoritative). You may ALSO compute g = WACC − (FCF / MarketCap) as a cross-check, but if your result differs by >1pp from the verified value, defer to verified and explain. If verified = N/A, write N/A — do NOT fabricate. SHOW arithmetic when data is available. |"
            "\n| 6-Month Return | X% |"
            "\n| Price vs 200-SMA | +X% |"
